...
if ENV['BEAKER_TESTMODE'] == 'agent' && ENV['VIRTUALBOX'] == 'yes'
  ...
  describe 'notifies with Virtualbox' do
    context 'test notifies' do
      ...
      describe command('knot-checkconf /etc/knot/knot.conf || cat /etc/knot/knot.conf'), node: dnsmaster do
        its(:stdout) { is_expected.to match %r{} }
      end

      describe command('knot-checkconf /etc/knot/knot.conf || cat /etc/knot/knot.conf'), node: dnsslave do
        its(:stdout) { is_expected.to match %r{} }
      end
