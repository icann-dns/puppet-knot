  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      describe 'check default config' do
        ...
        it do
          is_expected.to contain_concat__fragment('knot_key_test.example.com')
            .with(
              'content' => /test.example.com hmac-sha256 "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="/,
              'order'   => '02',
            )
        end
      end
      ...
      describe 'Change Defaults' do
        context 'algo' do
          ...
          it do
            is_expected.to contain_concat__fragment('knot_key_test.example.com')
              .with(
                'content' => /test.example.com hmac-md5 "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="/,
                'order'   => '02',
              )
          end
        end
        context 'data' do
          ...
          it do
            is_expected.to contain_concat__fragment('knot_key_test.example.com')
              .with(
                'content' => /test.example.com hmac-sha256 "foobar"/,
                'order'   => '02',
              )
          end
        end
